...
## Pembaruan setelah Tahap 02

### Perubahan status issue lama

| Issue | Status setelah Tahap 02 | Catatan |
|---|---|---|
| KI-0016 — Dependency boundary enforcement | PARTIALLY IMPLEMENTED | ESLint dan architecture tests kini menolak import test/audit serta `Math.random`; boundary antarmodul belum dapat diuji sebelum modul tersedia |
| KI-0018 — Static SPA rewrite | PARTIALLY VERIFIED | Vite dev deep route dan config test lulus; deployment Vercel nyata belum diuji |
| KI-0019 — Storage/private mode | PARTIALLY VERIFIED | Theme storage denial diuji; session dan IndexedDB belum berada dalam scope |
| KI-0022 — Lazy chunk version skew | OPEN | Loading dan route recovery tersedia, tetapi stale-deployment simulation belum ada |
| KI-0023 — Gate 2 tanpa codebase | RESOLVED AS BOOTSTRAP BLOCKER | Package, source, tests, coverage, dan build tersedia; Gate 2 kini diblokir domain, bukan bootstrap |

### KI-0024 — Kontrak domain dan mapping Model A belum dibuat

- **Prioritas:** P0 pada Tahap 03.
- **Status:** OPEN BY SCOPE.
- **Masalah:** Delapan elemen, delapan kanal, posisi, blok, quadra, 16 TIM, dan mapping Model A belum tersedia sebagai TypeScript contracts.
- **Risiko:** Gate 2 tidak dapat lulus dan modul berikutnya tidak memiliki vocabulary type-safe.
- **Kriteria tutup:** Seluruh finite domain mapping lengkap, invariant tests lulus, provenance dicatat, dan tidak ada React/DOM/storage dependency.

### KI-0025 — Tema dapat berubah setelah first paint

- **Prioritas:** P3 sebelum final UX gate.
- **Status:** OPEN.
- **Masalah:** Tema diselesaikan ketika React aktif; browser dapat menampilkan tema default sangat singkat sebelum preferensi tersimpan diterapkan.
- **Mitigasi saat ini:** `color-scheme`, theme-color update, dan system preference support.
- **Kriteria tutup:** Putuskan pre-hydration strategy yang kompatibel dengan security headers dan uji tanpa menduplikasi kontrak tema.

### KI-0026 — Browser dan accessibility matrix belum dijalankan

- **Prioritas:** P2 sebelum Gate 11.
- **Status:** OPEN.
- **Masalah:** RTL tests memeriksa accessible roles dan keyboard-operable controls, tetapi belum ada screen-reader review, real mobile browser test, zoom/reflow matrix, atau automated E2E.
- **Kriteria tutup:** Manual matrix terdokumentasi, keyboard-only flow lulus, screen-reader labels diperiksa, dan browser target disahkan.

### KI-0027 — Route retry memakai full reload

- **Prioritas:** P2 sebelum session persistence aktif.
- **Status:** ACCEPTABLE FOR BOOTSTRAP.
- **Masalah:** Recovery route dan root boundary saat ini menggunakan reload penuh.
- **Risiko masa depan:** Setelah sesi tersedia, reload harus mempertahankan checkpoint atau menjelaskan data yang belum tersimpan.
- **Kriteria tutup:** Integrasikan retry dengan versioned checkpoint dan compatibility policy pada Gate 9.
